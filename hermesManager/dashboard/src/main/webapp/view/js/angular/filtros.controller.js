(function() {
	...
	FiltrosController.$inject = [ '$scope', '$http', '$timeout', '$log',
			'$filter', 'eventsService' ];

	function FiltrosController($scope, $http, $timeout, $log, $filter,
			eventsService) {
...
//			  ];
		...
		function infoPopup(eventId, timestamp) {
			...
			datosEvento.innerHTML = '<b>EventId:</b> ' + eventId
					+ ' <b>Fecha:</b> ' + dateEvento + '<br/><b>Hora:</b> '
					+ hourEvento;
			return datosEvento;
		}
		...
		function pintarPuntos(events) {
			...
			angular.forEach(events, function(value, key) {
				...
				var latlng = L.latLng(value.position.coordinates[1],
						value.position.coordinates[0]);
				// Añado al mapa el punto
				var circle = L.circle(latlng, 5, mystyles).addTo(locations)
						.bindPopup(info);
			});
		}
		;
		...
		function pintarMapaMeasurements() {
			...
			var urlGet = "api/measurement/json/measurements?tipo="

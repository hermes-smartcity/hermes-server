package hermessensorcollector.lbd.udc.es.hermessensorcollector.bd;

/**
 * Clase con los nombre de las tablas y columnas usadas en la base de datos
 *
 * @author Leticia Riestra Ainsua
 *
 */
public class TablesDB {

    public static String TABLA_PARAMETERS = "parameters";
    public static String PARAMETERS_COLUMNA_ID = "id";
    public static String PARAMETERS_COLUMNA_NAME = "name";
    public static String PARAMETERS_COLUMNA_VALUE = "value";

    public static String TABLA_TAILSENDING = "tailsending";
    public static String TAILSENDING_COLUMNA_ID = "id";
    public static String TAILSENDING_COLUMNA_TYPE = "type";
    public static String TAILSENDING_COLUMNA_DATE = "date";
    public static String TAILSENDING_COLUMNA_ROUTEZIP = "routezip";
}
